...
`medium` (0.2 to 2 s), `slow` (>2 s or needs the network, so keep these on `pre-push`).
...
| [`gitguardian/ggshield`](https://github.com/GitGuardian/ggshield) | not verified | `ggshield` | medium | SaaS-backed; needs an API key. |
| [`thoughtworks/talisman`](https://github.com/thoughtworks/talisman) | not verified | `talisman-commit` | fast | Entropy + filename heuristics. |
...
| [`aquasecurity/trivy`](https://github.com/aquasecurity/trivy) | not verified | via `terraform_trivy`, or `repo: local` | slow | Image/filesystem/config scanning. Replaces the deprecated `tfsec`. |
| [`anchore/syft`](https://github.com/anchore/syft), [`anchore/grype`](https://github.com/anchore/grype) | not verified | `repo: local` | slow | SBOM + vulnerability match; better suited to CI than to a commit hook. |
| [`stackrox/kube-linter`](https://github.com/stackrox/kube-linter) | not verified | `kube-linter` | medium | Kubernetes manifest misconfiguration. |
| [`yannh/kubeconform`](https://github.com/yannh/kubeconform) | not verified | `kubeconform` | fast | Schema validation for K8s manifests. |

## 4. Infrastructure as code

| Tool | Version | Hook ids | Cost | Notes |
| --- | --- | --- | --- | --- |
| [`antonbabenko/pre-commit-terraform`](https://github.com/antonbabenko/pre-commit-terraform) | **1.109.1** (verified) | `terraform_fmt`, `terraform_validate`, `terraform_tflint`, `terraform_trivy`, `terraform_docs`, `terraform_providers_lock` | medium to slow | Since 1.109.0 each wrapped tool can be pinned with `--hook-config=--tool-version=X.Y.Z` (everything except checkov). |
...
| [`aws-cloudformation/cfn-lint`](https://github.com/aws-cloudformation/cfn-lint) | not verified | `cfn-lint` | medium | |
| [`terraform-linters/tflint`](https://github.com/terraform-linters/tflint) | not verified | via `terraform_tflint` | medium | |
...
| [`sethvargo/ratchet`](https://github.com/sethvargo/ratchet) | not verified | `repo: local` | fast | Pins/unpins action refs to SHAs; recognised by zizmor's version comments. |

## 6. SAST / language linters

| Tool | Version | Hook ids | Cost | Notes |
| --- | --- | --- | --- | --- |
| [`semgrep/semgrep`](https://github.com/semgrep/semgrep) | not verified | `semgrep` | slow | Rule-based SAST, `p/ci` and `p/secrets` registry packs. |
| [`PyCQA/bandit`](https://github.com/PyCQA/bandit) | not verified | `bandit` | medium | Python security linter. |
...
| [`shellcheck-py/shellcheck-py`](https://github.com/shellcheck-py/shellcheck-py) | not verified | `shellcheck` | fast | Shell script analysis. **Used by this bundle.** |
| [`scop/pre-commit-shfmt`](https://github.com/scop/pre-commit-shfmt) | not verified | `shfmt` | fast | Shell formatter. **Used by this bundle.** |
| [`pre-commit/mirrors-mypy`](https://github.com/pre-commit/mirrors-mypy) | not verified | `mypy` | slow | |
| [`biomejs/pre-commit`](https://github.com/biomejs/pre-commit) | not verified | `biome-check` | fast | JS/TS lint + format in one Rust binary. |
...
| [`pre-commit/pygrep-hooks`](https://github.com/pre-commit/pygrep-hooks) | not verified | `python-no-eval`, `text-unicode-replacement-char` | fast | |
| [`adrienverge/yamllint`](https://github.com/adrienverge/yamllint) | not verified | `yamllint` | fast | **Used by this bundle.** |
| [`crate-ci/typos`](https://github.com/crate-ci/typos) | not verified | `typos` | fast | Faster than `codespell`. **Used by this bundle.** |
| [`codespell-project/codespell`](https://github.com/codespell-project/codespell) | not verified | `codespell` | fast | |
| [`DavidAnson/markdownlint-cli2`](https://github.com/DavidAnson/markdownlint-cli2) | not verified | `markdownlint-cli2` | fast | |
| [`python-jsonschema/check-jsonschema`](https://github.com/python-jsonschema/check-jsonschema) | not verified | `check-github-workflows`, `check-dependabot`, `check-renovate` | fast | |
| [`dotenv-linter/dotenv-linter`](https://github.com/dotenv-linter/dotenv-linter) | not verified | `dotenv-linter` | fast | |

## 8. Commit messages

| Tool | Version | Hook ids | Cost | Notes |
| --- | --- | --- | --- | --- |
| [`commitizen-tools/commitizen`](https://github.com/commitizen-tools/commitizen) | not verified | `commitizen`, `commitizen-branch` | fast | Conventional Commits enforcement on the `commit-msg` stage. **Used by this bundle.** |
| [`jorisroovers/gitlint`](https://github.com/jorisroovers/gitlint) | not verified | `gitlint` | fast | Rule-based alternative. |
